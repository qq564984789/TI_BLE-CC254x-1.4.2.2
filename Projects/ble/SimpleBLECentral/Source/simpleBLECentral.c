 ...
 *                 HAL_KEY_SW_2
 *                 HAL_KEY_SW_1
 *          ÇÐ»»À¶ÑÀµÄ×´Ì¬ºÍ¹¦ÄÜ£¬¿ÉÒÔÊ¹ÓÃ´®¿ÚÀ´Ìæ»»°´¼ü!
 *          Central:  ÊÖ»ú£¬Ö÷ÒªÊÇÉ¨Ãè¡¢Á¬½Ó¹¤×÷
 * @return  none
 */
uint8 gStatus;
static void simpleBLECentral_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  if ( keys & HAL_KEY_UP )   //ÏòÉÏ°´¼ü
  {
    // Start or stop discovery   ¿ªÊ¼»òÍ£Ö¹Éè±¸·¢ÏÖ£¬¼´scanning
    if ( simpleBLEState != BLE_STATE_CONNECTED )  //Èç¹ûÎ´Á¬½Ó
    {
      ...
      {
        GAPCentralRole_CancelDiscovery();  //Í£Ö¹É¨Ãè
      }
    }  //Èç¹ûÒÑ¾­Á¬½Ó £¬Ôò¶ÁÈ¡»òÐ´ÌØÕ÷Öµ
  ...
  if ( keys & HAL_KEY_LEFT )   //Ïò×ó°´¼ü
  {
    // Display discovery results   ÏÔÊ¾É¨Ãèµ½µÄÉè±¸£¬ÔÚÉè±¸ÖÐ¹ö¶¯
    //É¨Ãèµ½µÄÉè±¸µØÖ·´æ·ÅÓÚsimpleBLEDevListÊý×éÖÐ
  ...
  if ( keys & HAL_KEY_CENTER )
  {
    ...
    if ( simpleBLEState == BLE_STATE_IDLE )  //Á¬½Ó
    {
      // if there is a scan result
      if ( simpleBLEScanRes > 0 )
      {
        // connect to current device in scan result   ´ÓÉ¨Ãèµ½µÄÉè±¸ÖÐÑ¡ÔñÉè±¸µØÖ·
  ...
  if ( keys & HAL_KEY_DOWN )  //ÏòÏÂ°´¼ü£¬Æô¶¯»ò¹Ø±Õ
  {
    // Start or cancel RSSI polling  Æô¶¯»òÖÕÖ¹RSSIµÄÖÜÆÚ¶ÁÈ¡£¬×¢ÒâÕâÊÇÔÚÁ¬½Ó×´Ì¬ÏÂ!
    // µ±È»£¬ÔÚÉ¨Ãè¹ý³ÌÖÐÒ²¿ÉÒÔ»ñµÃÉè±¸µÄRSSIÖµ£¬Ö»ÊÇµ÷ÓÃµÄAPI²»Í¬
    if ( simpleBLEState == BLE_STATE_CONNECTED )
    {
      if ( !simpleBLERssi )
      {
...
static uint8 simpleBLECentralEventCB( gapCentralRoleEvent_t *pEvent )
{
  switch ( pEvent->gap.opcode )
  {
    ...
    case GAP_DEVICE_INFO_EVENT:       //É¨Ãè¹ý³ÌÖÐÖ´ÐÐ
      { //GAP_DEVICE_INFO_EVENT:   ¸æÖªÖ÷»ú½ÓÊÕµ½´Ó»úµÄAdv Data»òÕßScan ResponseÏûÏ¢°ü
        ...
        if ( DEFAULT_DEV_DISC_BY_SVC_UUID == TRUE )
        {
          ...
          {  //deviceInfo:ÕâÊÇÒ»¸öÖØÒªµÄ±äÁ¿£¬¸ÃÀàÐÍÀïÃæ°üº¬rssiÖµ£¬ Èç¶ÁÈ¡´Ó»úµÄRSSIÖµ:
    ...
    case GAP_DEVICE_DISCOVERY_EVENT:   //É¨ÃèÍê³ÉºóÖ´ÐÐ£¬Éè±¸·¢ÏÖÊÂ¼þ
      {
        // discovery complete
