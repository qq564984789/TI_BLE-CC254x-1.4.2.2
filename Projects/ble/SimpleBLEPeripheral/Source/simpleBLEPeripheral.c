 ...
 *****************************************************************************/
/*
HAL_KEY_SW_2:¸Ä±äÉè±¸µÄ¹ã²¥×´Ì¬(´ò¿ª¡¢¹Ø±Õ)£¬ÎÒÃÇµÄÉè±¸ÉÏµçÄ¬ÈÏ´ò¿ª¹ã²¥
...
void SimpleBLEPeripheral_Init( uint8 task_id )
{
  ...
  {  //¸ù¾ÝÆ½Ì¨£¬ÉèÖÃÊÇ·ñÊ¹ÄÜ¹ã²¥.TI¹Ù·½Ìá¹©µÄ CC2540_MINIDK °åÄ¬ÈÏÊÇ²»´ò¿ª¹ã²¥µÄ
     //CC2540_MINIDK °åÊÇÍ¨¹ý°´¼üHAL_KEY_SW_2(ÔÚsimpleBLEPeripheral_HandleKeysÀï)´ò¿ªµÄ¡£
    #if defined( CC2540_MINIDK )
...
uint16 SimpleBLEPeripheral_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & SYS_EVENT_MSG )
  {
    ...
    // return unprocessed events     ·µ»ØÎ´´¦ÀíµÄÈÎÎñ±êÖ¾
  ...
  {
    // Start the Device
     /*Æô¶¯Éè±¸,À¨ºÅÄÚÎª»Øµ÷º¯Êý,À´ÉèÖÃÒªÏÔÊ¾µÄÐÅÏ¢»ò²Ù×÷*/
    VOID GAPRole_StartDevice( &simpleBLEPeripheral_PeripheralCBs );

    // Start Bond Manager
     /*ÉèÖÃ¶¨Ê±Ê±¼ä,µ½Ê±ºóÖÜÆÚÊÂ¼þµÄÈÎÎñid±»ÖÃÆð*/
  ...
  if ( events & SBP_PERIODIC_EVT )   /*¼ì²éÊÇ·ñÓÐÖÜÆÚÈÎÎñÊÂ¼þ*/
  {
    // Restart timer
      /*Èç¹ûÓÐÖÜÆÚÈÎÎñÊÂ¼þ*/
    ...
    // Perform periodic application task   /*´¦ÀíÖÜÆÚÊÂ¼þÖÐÒª×öµÄ¹¤×÷*/
    performPeriodicTask();
...
static void simpleBLEPeripheral_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  if ( keys & HAL_KEY_SW_2 )
  {
    ...
    if( gapProfileState != GAPROLE_CONNECTED )  //Î´Á¬½ÓÊ±
    {
#endif // PLUS_BROADCASTER
      uint8 current_adv_enabled_status;   //ÕâÁ½¸ö±äÁ¿Ê¼ÖÕÏà·´
      uint8 new_adv_enabled_status;

      //Find the current GAP advertisement status  
      GAPRole_GetParameter( GAPROLE_ADVERT_ENABLED, &current_adv_enabled_status );
	  //²é¿´µ±Ç°¹ã²¥ÊÇ·ñÊ¹ÄÜ:ÌîÈë²ÎÊýGAPROLE_ADVERT_ENABLED,±íÊ¾²éÑ¯µÄÊÇ¹ã²¥Ê¹ÄÜ
      if( current_adv_enabled_status == FALSE )  //Î´Ê¹ÄÜ
      {
        new_adv_enabled_status = TRUE;         //¿ªÆô
      }
      else                                      //Ê¹ÄÜ
      {
        new_adv_enabled_status = FALSE;        //¹Ø±Õ
      }
