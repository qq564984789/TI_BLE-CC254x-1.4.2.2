...
void osal_run_system( void )
{
  ...
  Hal_ProcessPoll();       //²éÑ¯Êý¾Ý :±ÈÈç´®¿ÚÊý¾Ý,usbÊý¾Ý
#endif /* USE_ICALL */
  ...
  do {                           //²»¶ÏÑ­»·²éÕÒÊÇ·ñÓÐÊÂ¼þ·¢Éú£¬ÈôÓÐÊÂ¼þ£¬Á¢¿ÌÍË³ödo¡£¡£¡£whileÑ­»·¡£appÓ¦ÓÃÓÅÏÈ¼¶×îµÍ
  ...
  {
    uint16 events;      //Ã¿¸öÈÎÎñ×î¶àº¬ÓÐ16¸öÊÂ¼þ
    halIntState_t intState;

    HAL_ENTER_CRITICAL_SECTION(intState);  //¹ØÖÐ¶Ï
    ...
    HAL_EXIT_CRITICAL_SECTION(intState);   //¿ªÖÐ¶Ï

    activeTaskID = idx;
    events = (tasksArr[idx])( idx, events );   //tasksArrÊÇÈÎÎñµÄ´¦Àíº¯ÊýÖ¸Õë
