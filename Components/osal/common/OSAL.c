...
static uint8 osal_msg_enqueue_push( uint8 destination_task, uint8 *msg_ptr, uint8 push )
{
  ...
  OSAL_MSG_ID( msg_ptr ) = destination_task;   //ÉèÖÃÏûÏ¢Êý¾ÝÊÇÊôÓÚÄÄ¸öÈÎÎñµÄ
  ...
  {
    // append the message
    osal_msg_enqueue( &osal_qHead, msg_ptr );  //½«Òª·¢ËÍµÄÏûÏ¢Êý¾Ý²åÈëµ½ÒÔosal_qHead¿ªÍ·µÄÊý¾ÝÁ´±íÖÐ
  }

  // Signal the task that a message is waiting   Í¨ÖªÖ÷Ñ­»·£¬ÓÐÈÎÎñÊÂ¼þµÈ´ý´¦Àí
...
uint8 osal_init_system( void )
{
  ...
  // Initialize the Memory Allocation System        ³õÊ¼»¯ÄÚ´æ·ÖÅä
  osal_mem_init();
#endif /* !defined USE_ICALL && !defined OSAL_PORT2TIRTOS */

  // Initialize the message queue                   ³õÊ¼»¯ÏûÏ¢¶ÓÁÐ
  osal_qHead = NULL;

  // Initialize the timers                             ³õÊ¼»¯¶¨Ê±Æ÷
  osalTimerInit();

  // Initialize the Power Management System        ³õÊ¼»¯µçÔ´¹ÜÀí
  osal_pwrmgr_init();
  ...
  osalInitTasks();                        //³õÊ¼»¯ÏµÍ³ÈÎÎñ£¬ÀïÃæÖ´ÐÐÁËBLE¸÷²ãµÄ³õÊ¼»¯£¬°üº¬×Ô¼ºappµÄ³õÊ¼»¯
 ...
 * @fn      osal_run_system   
 *
 * @brief   osalÊÇÒ»ÖÖ»ùÓÚÊÂ¼þÇý¶¯ µÄÂÖÑ¯Ê½²Ù×÷ÏµÍ³
 *     tasksEvents[idx]ÊÇÒ»¸öÊÂ¼þ±í£¬taskArr [idx]ÊÇÒ»¸öÊÂ¼þ´¦Àíº¯Êý±í
 *    ÂÖÑ¯ÊÂ¼þ±í£¬ÈôÓÐÊÂ¼þ·¢Éú£¬ÔòÌøµ½º¯Êý±íÖÐ¶ÔÓ¦µÄÊÂ¼þ´¦Àíº¯ÊýÖ´ÐÐ
...
//¼ì²éÏà¶ÔÓ¦µÄ±êÖ¾Î»£¬¾ÍÖ¸¶¨Ïà¶ÔÓ¦µÄÈÎÎñ¡£
void osal_run_system( void )
{
  uint8 idx = 0;      //ÊÂ¼þ±íµÄË÷Òý

#ifdef USE_ICALL
  uint32 next_timeout_prior = osal_next_timeout();
#else /* USE_ICALL */
#ifndef HAL_BOARD_CC2538
  osalTimeUpdate();         //¸üÐÂ¶¨Ê±Æ÷(ms¼¶ÐÄÌø¼ÆÊý),ÕâÀïµÄ"½ÚÅÄ"²»µ¥¶ÀÊ¹ÓÃÒ»¸ö¶¨Ê±Æ÷²úÉú
#endif                             //¶øÊÇÊ¹ÓÃµÄÃ¿320usÖÐ¶ÏÒ»´ÎµÄMAC¶¨Ê±Æ÷£¬ÕâÑù¾Í½«Èý¸ö¶¨Ê±Æ÷¿Õ³öÀ´¹©ÓÃ»§Ê¹ÓÃ
  Hal_ProcessPoll();       //²éÑ¯Ó²¼þ UART  ¡¢SPI   ¡¢  HID  µÄÊý¾Ý
#endif /* USE_ICALL */
  ...
  do {                           //Ñ­»·²é¿´ÊÂ¼þ±íÊÇ·ñÓÐÊÂ¼þ·¢Éú(Ã¿¸ö¶þ½øÖÆÎ»±íÊ¾Ò»¸öÊÂ¼þ)£¬ÈôÓÐÊÂ¼þ£¬Á¢¿ÌÍË³ödo¡£¡£¡£whileÑ­»·¡£appÓ¦ÓÃÓÅÏÈ¼¶×îµÍ
    if (tasksEvents[idx])  // Task is highest priority that is ready.      tasksEvents[idx] ÊÇÒ»¸öÖ¸ÕëÊý×é,´æ·ÅÁËÒ»¸öÈÎÎñÊÇ·ñ¸Ã±»ÔËÐÐµÄÐòÁÐ£¬ÄÇÃ´Õâ¸öÐòÁÐÊÇÈçºÎ²úÉúµÄÄØ ?
    {                                //´ð°¸£¬¾ùÊÇÍ¨¹ý osal_set_event()º¯Êý
      break;
    }
  } while (++idx < tasksCnt);       //tasksEvents[6]Í¨³£ÒÑ¾­ÊÇÓÃ»§×Ô¼º¶¨ÒåµÄÈÎÎñÊÂ¼þ

  if (idx < tasksCnt)     //Èç¹ûÕÒµ½ÁËÊÂ¼þ
  {
    uint16 events;      //Ã¿¸öÈÎÎñ×î¶àº¬ÓÐ16¸öÊÂ¼þ£¬ËùÒÔeventsÊÇÒ»¸ö16Î»ÀàÐÍ
    halIntState_t intState;

    HAL_ENTER_CRITICAL_SECTION(intState);  //±£´æÏÈÇ°ÖÐ¶Ï×´Ì¬£¬È»ºó¹ØÖÐ¶Ï
    events = tasksEvents[idx];        //¶ÁÈ¡¸ÃÈÎÎñËùº¬ÓÐµÄÊÂ¼þ(ÊÂ¼þ¿ÉÄÜ²»Ö¹1¸ö)
    tasksEvents[idx] = 0;  // Clear the Events for this task.   Çå³ýÊÂ¼þ¼ÇÂ¼£¬ÔÚÖ´ÐÐÈÎÎñ´¦Àíº¯ÊýÆÚ¼äÓÐ¿ÉÄÜ»áÓÐÊÂ¼þÔÙ´Î·¢Éú
    HAL_EXIT_CRITICAL_SECTION(intState);   //¿ªÖÐ¶Ï£¬»Ö¸´ÏÈÇ°ÖÐ¶Ï×´Ì¬

    activeTaskID = idx;                             // (tasksArr[idx])( idx, events );   ÕâÊÇÈÎÎñidxµÄÊÂ¼þ´¦Àíº¯Êý
    events = (tasksArr[idx])( idx, events );   //tasksArrÊÇÈÎÎñµÄ´¦Àíº¯ÊýÖ¸Õë£¬µ÷ÓÃÏàÓ¦µÄÈÎÎñÊÂ¼þ´¦Àíº¯Êý
                                                             //×¢Òâ: Ã¿Ò»¸öÈÎÎñÊÂ¼þ´¦Àíº¯Êý·µ»ØµÄÊÇÕâ¸öÈÎÎñÎ´±»´¦ÀíµÄÊÂ¼þ
    activeTaskID = TASK_NO_TASK;

    HAL_ENTER_CRITICAL_SECTION(intState);
    tasksEvents[idx] |= events;  // Add back unprocessed events to the current task. tasksEventsÊÇ¸öÖ¸Õë£¬Ö¸ÏòÄÚÈÝ : Ã¿¸öÈÎÎñËù°üº¬µÄ¸÷ÖÖÊÂ¼þ
	//°Ñ¸Õ²Å·µ»ØÎ´´¦ÀíµÄÊÂ¼þÌí¼Óµ½µ±Ç°ÈÎÎñÖÐ¼ÌÐø´¦Àí
	HAL_EXIT_CRITICAL_SECTION(intState);
  }
#if defined( POWER_SAVING ) && !defined(USE_ICALL)
  else  // Complete pass through all task events with no activity?
  {
    osal_pwrmgr_powerconserve();  // Put the processor/system into sleep    Ê¹ÏµÍ³Ë¯Ãß£¬ÒÔ±ã¸üµÍ¹¦ºÄ
  }
#endif
