...
/*********************************************************************
 * @fn      osalAddTimer    ÏòÁ´±íÀïÌí¼ÓÒ»¸ö"¶¨Ê±ÈÎÎñ"
 *
 ...
 * @return  none
 *********************************************************************/
void osalTimerUpdate( uint32 updateTime )   //Ã¿¸ô1  ms±»µ÷ÓÃÒ»´Î
{
  halIntState_t intState;
  osalTimerRec_t *srchTimer;
  ...
  if ( timerHead != NULL )
  {
    ...
    // Look for open timer slot
    while ( srchTimer )     //¶Ô  osal_start_timerExÉèÖÃµÄ"Èí¼þ¶¨Ê±Æ÷"¼õ¼ÆÊý
    {
           ...
           (srchTimer->reloadTimeout) && (srchTimer->event_flag) )
      {
        ...
        //Èç¹ûÄ³¸öµØ·½ÉèÖÃÁËÒ»¸öÈí¼þ¶¨Ê±Æ÷(osal_start_timerEx),µ±Õâ¸ö¶¨Ê±Ê±¼äµ½´ïºó£¬¾Í»áÉèÖÃÕâ¸öÊÂ¼þ
