 ...
 */
  ...
  /*                                              timeout_value  :¶¨Ê±Æ÷µÄ¶¨Ê±Ê±³¤,µ¥Î»ÊÇms
   * Set a Timer    Æô¶¯¶¨Ê±Æ÷£¬timeout_value Ê±¼äµ½ÁËÖ®ºó£¬²úÉúÒ»´Îevent
   */    
  extern uint8 osal_start_timerEx( uint8 task_id, uint16 event_id, uint32 timeout_value );
  
  /*
   * Set a timer that reloads itself.   Æô¶¯¶¨Ê±Æ÷£¬ ×Ô¶¯ÖØ×°:timeout_valuedÊ±¼äµ½ÁËÖ®ºó£¬¶¨Ê±²úÉúevent
   */
  extern uint8 osal_start_reload_timer( uint8 taskID, uint16 event_id, uint32 timeout_value );

  /*
   * Stop a Timer   Í£Ö¹¶¨Ê±Æ÷
   */
  extern uint8 osal_stop_timerEx( uint8 task_id, uint16 event_id );

  /*
   * Get the tick count of a Timer.   ·µ»ØÏµÍ³Ê±ÖÓ £¨Ò»°ãÓÃÓÚ¶Ô±ÈÏÖÔÚÓëÀúÊ·µÄÊ±¼ä£¬ ºóÃæÎÒÃÇµÄ AT ÃüÁî¾ÍÓÃµ½Õâ¸ö
À´¸¨ÖúÁË£©
   */
