/******************************************************************************
 ...
 @brief This file contains the interface to the H/W UART driver by ISR.
        Çë×¢ÒâÕâÀïµÄÎÊÌâ !
        Êµ²â : µ±bleÉ¨ÃèºÍ´®¿ÚÖÐ¶Ï¶¼Ê¹ÓÃµÄÊ±ºò£¬´®¿ÚÊý¾Ý½ÓÊÕÓÐÎÊÌâ
...
//²é¿´´®¿Ú½ÓÊÕÇøµ±Ç°¿ÉÒÔAVAILable µÄ×Ö½ÚÊý
//Ò²¾ÍÊÇ¿´½ÓÊÕ»º³åÇøÀïÃæÓÐ¶àÉÙÄÚÈÝ
//Ëã·¨: Î²- Ê×¡£  Èç¹ûÎ²ÅÜµ½Ê×Ç°ÃæÀ´ÁË£¬ÔòÊµ¼ÊµÄÎ²=
//HAL_UART_ISR_RX_MAX  + µ±Ç°Î²±êºÅ
...
// UxCSR - USART Control and Status Register.   ÕâÊÇUSARTµÄ¿ØÖÆ¼Ä´æÆ÷
...
// UxUCR - USART UART Control Register.  ×¢Òâ:Õâ½öÊÇ UART µÄ¿ØÖÆ¼Ä´æÆ÷
...
#define UTX0IE                     0x04        //´®¿Ú0 µÄ·¢ËÍÖÐ¶Ï Ê¹ÄÜÎ»
#define UTX1IE                     0x08       //´®¿Ú1 µÄ·¢ËÍÖÐ¶Ï Ê¹ÄÜÎ»
								    //×¢Òâ:CC2540µÄ´®¿Ú·¢ËÍ¡¢½ÓÊÕÖÐ¶Ï
								    //ÊÇ¶ÀÁ¢¿ªÀ´µÄ£¬¼ûuser guide   P44/47
#define P2DIR_PRIPO                0xC0

// Incompatible redefinitions result with more than one UART driver sub-module.
#undef PxOUT                                   //#undef   :È¥µôÔ­ÓÐµÄºê¶¨Òå
#undef PxDIR
#undef PxSEL
#undef UxCSR
#undef UxUCR
...
#undef IENx
#undef IEN_BIT
#if (HAL_UART_ISR == 1)          //Ñ¡ÔñÊ¹ÓÃP0»¹ÊÇP1¿Ú×÷Îª´®¿ÚÊ¹ÓÃ
#define PxOUT                      P0
#define PxIN                       P0
#define PxDIR                      P0DIR
#define PxSEL                      P0SEL
...
//½«Ä³Ò»GPIO¿Ú×öÍâÉèÊ¹ÓÃÊ±£¬Á½²½×ß:
// 1¡¢CC2430µÄÆ¬ÉÏÍâÉè¾ù¿ÉÓ³Éäµ½GPIOµÄÁ½×é±¸ÓÃÎ»ÖÃ´¦£¬ÎÒÃÇÒªÑ¡ÔñÆäÖÐÒ»¸ö¡£
//PERCFG:¶ÔÓÚÒ»¸öÍâÉè(USART ¡¢Timer)£¬½«ËüÓ³Éäµ½µÚ1Î»ÖÃ»¹ÊÇ
//            µÚ¶þÎ»ÖÃ    ¼ûuser guide   P88
// USART0 ÓÉÓÚÑ¡ÓÃ Alt-1£¬Òò´ËÒªÇåÁã(Î»Óë0->Î»Óë·Ç1:¿ÉÒÔ²»¸Ä±äÆäËûÎ»)
// USART1Ñ¡ÓÃ Alt-2£¬Òò´ËÒªÖÃÎ»(Î»»ò1)
// 2¡¢½«±»ÍâÉèÑ¡ÖÐµÄGPIO¿Ú±ä³ÉÍâÉè¹¦ÄÜ£¬Ê¹ÍâÉè¹¦ÄÜÉúÐ§   P84  ±í7-1
// ÏÂÃæµÄHAL_UART_Px_RX_TX¶¼ÊÇ¶Ô P0SEL,P1SELÉèÖÃ£¬¼ûP88       
#if (HAL_UART_ISR == 1)  
...
#endif
//´®¿ÚÊý¾Ý½ÓÊÕ /·¢ËÍ»º³åÇøµÄµÄÊý¾Ý½á¹¹
//´®¿ÚµÄ¶ÁÐ´º¯Êý²»ÊÇÕæÕýµÄ·¢ËÍ½Ó¸Äº¯Êý,
//ËûÃÇÖ»ÊÇ¸ºÔð½«ÊýÌîÈëÏÂÃæµÄ»º³åÇø,»òÕß´Ó»º³åÇøÖÐ¶Á³ö
//headºÍtail¶¼ÊÇÒ»¸ö·½ÏòÒÆ¶¯,µ±ÒÆµ½Ä©Î»¾Í»Øµ½¿ªÍ·Î»ÖÃ.
//rxBufºÍtxBufÊÇÒ»¸öÑ­»·Êý×é£¬ÀïÃæÄÚÈÝÓÉHead¡¢ TailÁ½¸öÓÎ±êÖ¸Ê¾
//µ±ÓÎ±êÖ¸Ê¾µ½Êý×éÎ²Ê±£¬ÓÎ±ê±äÎª0£¬ÔÙ´ÓÍ·²¿¿ªÊ¼
// ¶ÁÐ´º¯ÊýÖ»ÊÇ½«ÊýÌîÈë»ò¶Á³ö»º³åÇø,È»ºóÒÆ¶¯ÆäÖÐÒ»¸öÖ¸Õë..
//ÄÇÃ´Ê²Ã´Ê±ºò½áÊøÄØ? ¾ÍÊÇhead==tail..
typedef struct
{
  uint8 rxBuf[HAL_UART_ISR_RX_MAX];
  rxIdx_t rxHead;
  volatile rxIdx_t rxTail;
  uint8 rxTick;        //´®¿Ú½ÓÊÕÊý¾Ý³¬Ê±±êÖ¾
  uint8 rxShdw;      //´®¿Ú½ÓÊÕÊý¾Ý³¬Ê±±êÖ¾

  uint8 txBuf[HAL_UART_ISR_TX_MAX];
  volatile txIdx_t txHead;
  txIdx_t txTail;
  uint8 txMT;   //´®¿ÚÊý¾Ý·¢ËÍÍê³É±êÖ¾
...
static void HalUARTOpenISR(halUARTCfg_t *config)
{
  ...
  URXxIE = 1;                                   //´ò¿ª´®¿ÚµÄ½ÓÊÕ ÖÐ¶Ï Ê¹ÄÜ
  UTXxIF = 1;  // Prime the ISR pump.    // ÉèÖÃ´®¿ÚµÄ·¢ËÍÖÐ¶Ï£¬Ò»µ©´®¿Ú·¢ËÍÖÐ¶ÏÊ¹ÄÜ´ò¿ª£¬
                                                      //¾Í¿ÉÍê³ÉÒ»´Î´®¿ÚÊý¾Ý·¢ËÍ
  UxCSR = (CSR_MODE | CSR_RE);      //USARTÉèÖÃÎªUARTÄ£Ê½£¬ÇÒÔÊÐí½ÓÊÕ¡£×¢Òâ:
                                                     //Ö»ÓÐÔÚ´®¿ÚÅäÖÃºÃºó£¬²ÅÔÊÐí½ÓÊÕÊý¾ÝP168
}
...
static uint16 HalUARTReadISR(uint8 *buf, uint16 len)
{
  ...
  {
    *buf++ = isrCfg.rxBuf[isrCfg.rxHead++];       //¶ÁÊý¾Ý´Ó»º³åÇøµÄÍ·²¿ÓÎ±ê¿ªÊ¼
...
static uint16 HalUARTWriteISR(uint8 *buf, uint16 len)
{
  ...
  // Enforce all or none.
  if (HAL_UART_ISR_TX_AVAIL() < len)    //Èç¹û·¢ËÍ»º³åÇøÊ£Óà¿ÉÐ´µÄ¿Õ¼äÐ¡ÓÚlen
  {
    return 0;
  }

  for (cnt = 0; cnt < len; cnt++)       //Ð´Êý¾Ý´Ó»º³åÇøµÄÎ²²¿ÓÎ±ê¿ªÊ¼
  {
    ...
    IEN2 |= UTXxIE;              //×¢Òâ£¬ÕâÀïÊÇÔÚforÑ­»·ÖÐ²»¶Ï
	//´ò¿ª´®¿Ú·¢ËÍÖÐ¶ÏµÄÊ¹ÄÜ,ÓÉÓÚÇ°ÃæÒÑ¾­´ò¿ªÁË·¢ËÍÖÐ¶Ï
	//Òò´Ë£¬ÔÚÕâÀï»á´¥·¢Ò»´Î·¢ËÍÖÐ¶Ï£¬ÌøÖÁHAL_ISR_FUNCTION( halUart0TxIsr, UTX0_VECTOR )
  }
...
#endif
{
  HAL_ENTER_ISR();      //´ò¿ª×ÜÖÐ¶Ï

  if (isrCfg.txHead == isrCfg.txTail)  //
  {
    IEN2 &= ~UTXxIE;         //¹Ø±Õ´®¿Ú·¢ËÍÖÐ¶ÏµÄÊ¹ÄÜ£¬ÒòÎªºóÃæ²»ÔÙ·¢ËÍ
    isrCfg.txMT = 1;           //·¢ËÍÍê³É±êÖ¾Î»
  }
  else
  {
    UTXxIF = 0;              //½øÈë·¢ËÍÖÐ¶Ïºó£¬Òª¹Ø±Õ·¢ËÍÖÐ¶Ï
    UxDBUF = isrCfg.txBuf[isrCfg.txHead++];       //½«»º³åÇøµÄ´ø·¢ËÍÄÚÈÝ·ÅÖÁUxDBUF
                                                                     //×¢Òâ£¬µ±UxDBUFÄÚÈÝÃ»ÓÐÖ®ºó
                                                                     //ÓÖ»áÒýÆðÒ»´Î·¢ËÍÖÐ¶Ï
